...
public class ActivateUserTest {
	...
	public void setUp() {
		...
		this.activateUser = new ActivateUser(this.userDAO);
		this.user = new User(
				UUID.randomUUID(), 
				new Date(), 
				new Date(), 
	...
	public void test_execute_givenAnErrorWhenGetUserObject_expectThrowUpdateUserException() throws HpgMethylException {
		...
		this.activateUser.execute(request);
	}
	...
	public void test_execute_givenValidRequest_expectUser() throws HpgMethylException {
		...
		ActivateUserResponse useCaseResponse = this.activateUser.execute(request);
		
		User response = useCaseResponse.getUser();
		
		Assert.assertTrue(response.getActive());
	}
}
